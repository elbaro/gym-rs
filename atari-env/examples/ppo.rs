/*
A modified example from tch-rs
https://github.com/LaurentMazare/tch-rs/blob/master/examples/reinforcement-learning/ppo.rs
*/
...
pub fn train() -> anyhow::Result<()> {
    let available_actions = env.minimal_actions();
    println!("action space: {}", env.action_space());
    println!("observation space: {:?}", env.observation_space());

    let device = tch::Device::cuda_if_available();
    let vs = nn::VarStore::new(device);
    let model = model(&vs.root(), available_actions.len());
